...
Pre-Phase-1 conexus authenticated two distinct populations with one
...
3. **CLI** — `conexus router create-operator --username <u>` for
...
- The dashboard's cookie path now reaches the backend via a
  router-signed `X-CoNexus-Forwarded-Operator` header (PR #209)
