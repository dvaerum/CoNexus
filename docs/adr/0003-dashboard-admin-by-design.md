...
repo's `common/user/conexus/docs/adr/` (as `0003-dashboard-admin-by-design.md`)
rather than here, back when this fork had no `docs/adr/` of its own.
Moved here 2026-09-06 since it records a decision about conexus's
...
The dashboard has no login layer and never will at the conexus
level. Anyone who can reach `/conexus/__dashboard/<name>/` is
...
per-user authentication into conexus, which would multiply scope.
