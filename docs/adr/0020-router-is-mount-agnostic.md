...
serves every route at the host **root** as well as under `/conexus`
(additive aliases), derives the external prefix/origin per request
(`conexus/router/mount.py`), gates root-aliased routes on the
...
`/conexus` URLs in the served page, connected, no console errors.
**Date**: 2026-07-31
**Amends / supersedes-in-part**:
  ...
  - **ADR-0014** (REST admin API) — only the leading `/conexus` in its
...
The always-on router serves every route under a hardcoded `/conexus`
prefix — `GET /conexus/api/router/health`, `/conexus/app/<project>/`,
`/conexus/<project>/mcp`, `/conexus/assets`, and so on. The literal
`/conexus` is baked into ~20 source files (25 occurrences in
...
> `/conexus/`. The operator's correct mental model — "the `/conexus`
...
same time, **both**
  ...
  - `https://nixos-developer-system.<tailnet>/conexus/…`
    (tailscale-serve, mounted under **`/conexus`**).
...
1. **Serve at root.** Route registration drops the `/conexus` prefix:
   ...
   project routes; only the outer `/conexus` is removed.
...
2. **The proxy owns the prefix.** A proxy that co-hosts services mounts
   ...
   - tailscale-serve / nginx at `/conexus`: strip `/conexus`, send
     `X-Forwarded-Prefix: /conexus`.
...
**Negative**
- **Large blast radius**: ~20 files hardcode `/conexus`; every route
...
1. **Derive-and-thread**: add per-request external-identity derivation
   ...
   No route changes yet; the helper returns `/conexus` by default so
   behavior is unchanged.
2. **Serve at root + strip at the proxy**: drop the `/conexus` prefix
   from route registration; update the two production proxies
   (tailscale-serve TLS config + the nginx module) to **strip
   `/conexus` and send `X-Forwarded-Prefix: /conexus`**. External URLs
