# Restore preload fixture bundle(s) into the conexus state dir on a
...
# (conexus_preload=<name>[,<name>...]), placed there by
...
# Parse conexus_preload=<names> from the kernel cmdline. Last wins if
...
for tok in "${_cmdline[@]}"; do
  case "$tok" in
    conexus_preload=*) names="${tok#*=}" ;;
  esac
done
if [ -z "$names" ]; then
  echo "conexus-vm-dev-preload: no conexus_preload= on kernel cmdline; nothing to do"
  exit 0
fi
...
if [ -e "$marker" ]; then
  echo "conexus-vm-dev-preload: already applied ($(cat "$marker" 2>/dev/null || true)); skipping"
  exit 0
fi
...
for name in "$@"; do
  ...
  if [ ! -f "$bundle" ]; then
    echo "conexus-vm-dev-preload: FAIL — bundle '$name' not found at $bundle" >&2
    avail=""
    ...
    echo "conexus-vm-dev-preload: available bundles: ${avail:-<none baked into this image>}" >&2
    exit 1
  fi
  echo "conexus-vm-dev-preload: restoring '$name' -> $state_dir"
  # -p perms, --same-owner: the fixture was captured from this same VM
  # image so the archived uid/gid match the runtime conexus user.
  tar -C "$state_dir" --zstd -xpf "$bundle" -p --same-owner
  applied="${applied:+$applied,}$name"
done

printf '%s\n' "$applied" >"$marker"
echo "conexus-vm-dev-preload: done — applied [$applied] into $state_dir"
