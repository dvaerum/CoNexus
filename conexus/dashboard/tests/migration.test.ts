 ...
 * Asserts that:
 *
 *   * ``conexus/dashboard/lib/api/*.ts`` no longer splices
 ...
 * Python backend (``conexus/app/routers/*.py``, ``conexus/router/
 * app.py``) for the same body-token-read pattern, plus a TODO-marker
 * sweep over the whole ``conexus/**\/*.py`` tree — all three deleted
